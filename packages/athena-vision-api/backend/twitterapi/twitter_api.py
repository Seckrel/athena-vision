...
class TwitterAPI:
    def __init__(self) -> None:
        self.api_key = getenv("T_API_KEY")
        self.api_secret = getenv("T_API_KEY_SECRET")
        self.client_id = getenv("T_CLIENT_ID")
        self.client_secret = getenv("T_CLIENT_SECRET")
        self.oauth_callback_url = getenv("T_CALLBACK_URL")
    ...
    def get_me(self, access_token, access_token_secret):
        try:
            client = tweepy.Client(
                consumer_key=self.api_key, consumer_secret=self.api_secret, access_token=access_token,
